...
export const createUi = (layer: Layer, sliceName: string) => {
  ...
  const createUiStructure = () => {
    try {
      ...
      if (e instanceof Error) {
        throw new Error(`Failed to create ui structure: ${e.message}`)
      }
  ...
  const createUiComponent = () => {
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        storiesTemplate(layer, componentName, isDefaultExport)
      )
      if (layer === 'pages') {
        fs.writeFileSync(
      ...
      if (e instanceof Error) {
        throw new Error(`Failed to create ui component: ${e.message}`)
      }
      throw new Error('Unexpected error')
    }
  }
